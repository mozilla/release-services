...
def file_instance_exists(app, digest, exp_regions):
    """test for intersection between file regions and expected regions"""
    with app.app_context():
        tbl = tables.File
        file = tbl.query.filter(tbl.sha512 == digest).first()
        regions = [i.region for i in file.instances]
...
@moto.mock_s3
@test_context
def test_remove_file_pending_deletion(app):
    """The file should not have any active regions once it has expired"""
    instances = ['us-east-1']
    with app.app_context():
        make_key(app, 'us-east-1', 'tt-use1', util.keyname(DATA_DIGEST), DATA)
