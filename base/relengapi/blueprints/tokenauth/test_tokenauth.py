...
actions.test_tokenauth.zig.doc("Zig")
actions.test_tokenauth.zag.doc("Zag")
...
def insert_token(app):
    ...
    t = Token(
        id=1, actions=[actions.test_tokenauth.zig], description="Zig only")
    session.add(t)
    session.commit()
...
@test_context.specialize(actions=[])
...
@test_context.specialize(actions=[actions.base.tokens.view])
...
@test_context.specialize(actions=[actions.base.tokens.issue,
                                  actions.test_tokenauth.zig])
def test_issue_token_success(client):
    """Successful token issuance returns a token"""
    request = {'actions': ['test_tokenauth.zig'], 'description': 'More Zig'}
...
def test_issue_token_not_subset(client):
    """Tokens can only allow actions the issuer can perform, failing with
    'bad request' otherwise."""
    request = {'actions': ['test_tokenauth.zig', 'test_tokenauth.zag'],
...
def test_issue_token_no_description(client):
    ...
    request = {'actions': ['test_tokenauth.zig']}
    eq_(client.post_json('/tokenauth/tokens', request).status_code, 400)


@test_context.specialize(actions=[], db_setup=insert_token)
...
def test_get_token_exists(client):
    """Getting an existing token returns its id, actions, and description."""
...
@test_context.specialize(actions=[actions.base.tokens.view], db_setup=insert_token)
...
@test_context.specialize(actions=[actions.base.tokens.revoke],
                         db_setup=insert_token)
...
def test_loader_no_header(app, client):
    """With no Authentication header, no actions are allowed"""
...
def test_loader_not_bearer(app, client):
    """With an Authentication header that does not start with 'Bearer', no actions are allowed"""
    auth = json.loads(
...
def test_loader_good_header(app, client):
    """With a good Authentication header, the actions in the DB are allowed"""
    auth = json.loads(
        client.get('/test_tokenauth',
                   headers=[('Authentication', 'Bearer TOK/1/v1')]).data)
    eq_(auth['provides'], ['test_tokenauth.zig'])


@test_context.specialize(db_setup=insert_token)
def test_loader_bad_header(app, client):
    """With a bad Authentication header, no actions are allowed"""
    auth = json.loads(
        client.get('/test_tokenauth',
...
def test_loader_good_header_not_in_db(app, client):
    """With a good Authentication header but no row in the DB, no actions are allowed"""
    auth = json.loads(
        client.get('/test_tokenauth',
