                   ...
                   hook =
                     mkTaskclusterHook
                       { name = hook_name;
                         ...
                         deadline = "1 hour";
                         maxRunTime = 2 * 60 * 60;  # 2 hours
                         schedule = if channel == "production"
                                    then [ "0 0 * * *" ]  # every day at midnight
                                    else [];
                         scopes =
                         ...
                         taskCommand = [
                           ...
                           (builtins.concatStringsSep " && " [
                             ...
                             "./please -vv tools update-dependencies ${project_name} --branch-to-push=${branch} --taskcluster-secret='repo:github.com/mozilla-releng/services:branch:${channel}'"
                           ])
                         ];
                         workerType = "releng-svc";
                       };
                in { name = channel;
                     value = writeText "taskcluster-hook-${hook_name}.json" (builtins.toJSON hook);
                   })
              [ "testing"
                "staging"
                "production"
