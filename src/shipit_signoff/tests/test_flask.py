...
def mocked_getinfo(fields, access_token):
    if access_token == 'goodtoken':
        return backend_common.mocks.AUTH0_DUMMY_USERINFO
    else:
...
def test_login(client):
    ...
    resp = client.get('/login', query_string={'callback_url': '/step'})
...
def test_get_missing_step(client):
    resp = client.get('/step/{}'.format(INVALID_UID), headers=GOODHEADERS)
    assert resp.status_code == 404


def test_get_present_step(client):
    resp = client.get('/step/{}'.format(UID), headers=GOODHEADERS)
    assert resp.status_code == 200
...
def test_get_step_status(client):
    resp = client.get('/step/{}/status'.format(UID), headers=GOODHEADERS)
    assert resp.status_code == 200
    data = json.loads(resp.data)
...
def test_delete_missing_step(client):
    resp = client.delete('/step/{}'.format(INVALID_UID), headers=GOODHEADERS)
    assert resp.status_code == 404


def test_delete_step_bad_token(client):
    resp = client.delete('/step/{}'.format(UID), headers=BADHEADERS)
    assert resp.status_code == 401
