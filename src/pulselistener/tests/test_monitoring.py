...
async def test_monitoring(QueueMock, NotifyMock):
    bus = MessageBus()
    monitoring = Monitoring(['pinco@pallino'], 1, 'user', 'token')
    monitoring.register(bus)
    await bus.send('monitoring:in', ('Group1', 'Hook1', 'Task-invalid'))
    await bus.send('monitoring:in', ('Group1', 'Hook1', 'Task-pending'))
    await bus.send('monitoring:in', ('Group1', 'Hook1', 'Task1-completed'))
    await bus.send('monitoring:in', ('Group1', 'Hook1', 'Task2-completed'))
    await bus.send('monitoring:in', ('Group1', 'Hook2', 'Task-exception'))
    await bus.send('monitoring:in', ('Group2', 'Hook1', 'Task-failed'))
...
async def test_monitoring_whiteline_between_failed_and_hook(QueueMock, NotifyMock):
    ...
    await bus.send('monitoring:in', ('Group1', 'Hook1', 'Task-failed'))
    await bus.send('monitoring:in', ('Group1', 'Hook2', 'Task-failed'))
    assert bus.queues['monitoring:in'].qsize() == 2
...
async def test_monitoring_retry_exceptions(QueueMock, NotifyMock):
    ...
    await bus.send('monitoring:in', ('Group1', 'Hook1', 'Task-exception-retry:2'))
    await bus.send('monitoring:in', ('Group1', 'Hook2', 'Task-exception-retry:0'))
    assert bus.queues['monitoring:in'].qsize() == 2
...
async def test_monitoring_restartable(QueueMock, IndexMock):
    ...
    assert len(monitoring.queue.created_tasks) == 0
    await bus.send('monitoring:in', ('Group', 'Hook-staticanalysis/bot', 'Task-failed-restart'))
    await monitoring.check_task()
    assert len(monitoring.queue.created_tasks) == 1
