...
type alias HawkParameters =
    { -- Simple structure to store parameters
      ...
      backend : UrlMethod
    , -- Build an hawk header for backend auth
      target : Maybe UrlMethod
    , -- Build an optional hawk header for a sub target hit from backend (TC)
      body : Maybe String
    , requestType : HawkRequestType
    }
    ...
    case model.user of
        Just user ->
            let
                ...
                workflows =
                    Dict.insert wid workflow model.workflows
            in
        ...
        Nothing ->
            ...
            let
                l =
                    Debug.log "Skipping request" params
            in
