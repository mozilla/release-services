...
from relengapi_common.utils import synchronized
...
class Alchemies(object):
    ...
    def _get_db_config(self, dbname):
        ...
        if uris is not None:
            ...
            self.app.log.warning(
                "Using URI {} for database {}".format(uris[dbname], dbname))
            return uris[dbname]
        else:
            ...
            self.app.log.warning(
                "Using URI {} for database {}".format(uri, dbname))
            return uri
        ...
        if dbname not in self._engines:
            ...
            try:
                create_engine = getattr(
                    self, 'create_{}_engine'.format(dialect))
    ...
    def create_mysql_engine(self, url):
        ...
        def check_client_charset(dbapi_con, con_record):
            # older versions of the MySQL default to `latin1` for all of the
            # client-derived charsets.  That's no fun.  The charset=utf8 above
            # should fix this, but it's best to be sure.
            curs = dbapi_con.cursor()
            curs.execute("show variables like 'character_set_client'")

            if curs.fetchone()[1] != 'utf8':
                raise RuntimeError(
                    "MySQL client connection is not using utf8 encoding")
...
def init_app(app):
    ...
    for dbname in db.database_names:
        log.info("creating tables for database %s", dbname)
